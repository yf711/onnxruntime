...
Status DepthToSpaceOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder,
  ...
  if (model_builder.CreateMLProgram()) {
    ...
    if (mode == "DCR") {
      ...
      AddOperationInput(*op, "blocksize", model_builder.AddScalarConstant(op->type(), "blocksize", blocksize));
      ...
      // is a fixed size hopefully CoreML is smart enough to do that during model compilation.
      ...
      std::vector<int64_t> shape1 = {input_shape[0],
      ...
      // transpose to [0, 1, 4, 2, 5, 3]
      auto transpose = model_builder.CreateOperation(node, "transpose");
      std::vector<int64_t> perm = {0, 1, 4, 2, 5, 3};
      std::vector<int64_t> shape2 = {shape1[0], shape1[1], shape1[4], shape1[2], shape1[5], shape1[3]};
