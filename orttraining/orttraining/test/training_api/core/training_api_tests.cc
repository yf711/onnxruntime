...
std::vector<uint8_t> ReadFileIntoBuffer(const PathString& file_path) {
  size_t num_bytes = 0;
  ORT_THROW_IF_ERROR(Env::Default().GetFileLength(file_path.c_str(), num_bytes));
  ...
  ORT_ENFORCE(bytes_stream);

  return buffer;
}

gsl::span<const uint8_t> ReadFileIntoSpan(const PathString& file_path) {
  std::vector<uint8_t> buffer = ReadFileIntoBuffer(file_path);
  return gsl::make_span(reinterpret_cast<const uint8_t*>(buffer.data()), buffer.size());
}
...
void TestModuleExportFromBuffer(const std::vector<std::shared_ptr<IExecutionProvider>>& providers) {
  ...
  gsl::span<const uint8_t> training_model_span = ReadFileIntoSpan(training_model_uri);
  gsl::span<const uint8_t> eval_model_span = ReadFileIntoSpan(eval_model_uri);
  ...
  std::unique_ptr<Environment> env;
  ASSERT_STATUS_OK(Environment::Create(nullptr, env));
  auto model_identifier = ModelIdentifiers(training_model_span,
                                           eval_model_span,
                                           std::nullopt);
  auto model = std::make_unique<onnxruntime::training::api::Module>(
      model_identifier, &state, onnxruntime::SessionOptions(),
